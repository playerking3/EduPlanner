...
class Turma:
    ...
    def getList(self):
        query = "SELECT turma.id_turma, turma.id_curso, turma.id_sala, turma.nome, DATE_FORMAT(turma.inicio, '%Y-%m-%d') as inicio, DATE_FORMAT(turma.fim, '%Y-%m-%d') as fim, turma.lista_dias, TIME_FORMAT(turma.horas_dia, '%H:%i') as horas_dia, turma.status, TIME_FORMAT(turma.hora_inicio, '%H:%i') as hora_inicio FROM `turma`;"
        conexao = Conection()
